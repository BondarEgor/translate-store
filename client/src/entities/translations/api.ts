...
export const translationsApi = {
  ...
  remove: (locale: string, key: string) =>
    api.delete<void>(`/api/translations/${locale}/${key}`),

  updateAll: ({ value, key, namespace, locale }: { namespace: string; locale: string; value: string; key: string; }) => {
    return api.put<string>(`/api/translations/${namespace}/${locale}/${key}/all`, {
      value
    });
  },

  update: ({ value, key, namespace, locale }: { namespace: string; locale: string; value: string; key: string; }) => {
